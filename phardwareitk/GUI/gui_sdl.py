...
def GetHwnd(title:str) -> Optional[Any]:
    """Returns the windows handle for the specific window with the specified title.

    Args:
        title (Any): The title of the created window.
...
def TitleBarWindows(title:str, mode:Optional[str]=None, color:Color=Color("white")) -> None:
    """Modifies the orignal titleBar, to the specified color or mode. WINDOWS SPECIFIC

    Args:
        title (str): the title of the window, that needs to be modified.
...
class TitleBar():
    ...
    def __init__(self, window:Any, width:Optional[int]=None, height:int=50, title:str="PHardwareITK", bgcolor:Color=Color("black")) -> None:
        ...
        self.window = window
        self.colors = {"bg": bgcolor, "text": Color("white")}
        ...
        self.interactivity = {"draggable": True, "close_action": None, "on_click": None}
    ...
    def Draw(self, renderer:SDL_Renderer) -> None:
        ...
        SDL_RenderFillRect(renderer, SDL_Rect(0, 0, self.size["width"], self.size["height"]))
        ...
        # Additional stuff
        if self.interactivity["draggable"]:
            pass
...
class Shape(PWidget):
    ...
    def Draw(self, renderer:SDL_Renderer, window:Any) -> SDL_Renderer:
        """Draws the Shape.

        Args:
            renderer (SDL_Renderer): The renderer to use.
            window (Any): The window to draw upon.
...
class AdvQuadrilateral(PWidget):
    ...
    def Draw(self, window:Any) -> SDL_Renderer:
        ...
        Returns:
            SDL_Renderer: The renderer of the shape.
        """
        return Shape(((self.p1, self.p1w), (self.p2, self.p2w), (self.p3, self.p3w), (self.p4, self.p4w)), self.x, self.y, self.wireframe, self.bgcolor, self.color).Draw(window)