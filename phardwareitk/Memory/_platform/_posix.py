    ...
    def stop_proc(self, pid:int):
        ...
        with self.lock:
            if pid in self.procs:
                ...
                # Explicitly remove from queue to prevent stale references
                if not self.ready_queue.empty():
                    self.ready_queue.queue = [p for p in self.ready_queue.queue if p.pid != pid]
