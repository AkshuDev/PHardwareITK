...
def setup_dir_tree(_file_manager:object, _cd_:str, _user:str) -> tuple[str, str]:
    ...
    out = _file_manager.get_file(aosdusr, __file__)
	...
	if out is not None:
		...
		out = _file_manager.write_file(user_config, json.dumps(user_config_data).encode("utf-16"), __file__, [0x0, 0x5, 0x6, [None]])
		
		if not out == 0:
			print("Error,", out)
		
		out = _file_manager.write_file(aosdusr, json.dumps(aosdusr_data).encode("utf-8"), __file__, [0x0, 0x5, 0x6, [None]])
