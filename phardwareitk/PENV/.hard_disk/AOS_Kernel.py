...
_cd_, _user, _cd_backup = setup_dir_tree(_file_manager, _cd_, _user)
...
while True:
	...
	if cmd[0] == "poweroff" or cmd[0] == "shutdown":
		...
		if cmd_aos == "wipe-drive":
			...
			out = _file_manager.write_file(b"home\\aosduser.aos", json.dumps({"usrf": f"home\\usrs\\{usr}_conf.aos"}).encode("utf-8"), [0x0, 0x5, (os_files, None)])
			if not out == 0:
			...
			for part in new_dir_l:
				if part == "..":
					...
					if not cd_backupt == "home":
						...
						cd_backupL.pop(len(_cd_backupL) - 1)
						cd_backupt = "".join(*_cd_backupL)
				else:
					_cd_backup_t = _cd_
					_cd_t = self._file_manager.join_path(cd_t.encode("utf-8"), part.encode("utf-8")).decode("utf-8")
					out = self._file_manager.dir_exists(_cd_t.encode("utf-8"))
					if out == False:
