...
class PBFS_DriveData:
    ...
    def file_table_handler(self, offset: int, data: bytes, write: bool = False, entries: int=0, file_name: bytes=b"", file_data_offset: int=0, block_span: int=0, permission_offset: int=0) -> Union[list, bytes]:
        ...
        if not write:
            ...
            if data == b"" or data[offset:self.file_table_entry_size * entries] == b"": return []

            file_table_data = data[offset:self.file_table_entry_size * entries] # Get file table data
            for entry in range(entries):
                print(file_table_data[entry * self.file_table_entry_size : (entry + 1) * self.file_table_entry_size])
    ...
    def permission_table_handler(self, offset: int, data: bytes, write: bool = False, entries:int=0, permissions:list=[0x0, 0x1], file_tree_offset: int=0) -> Union[list, bytes]:
        ...
        if not write:
            if data == b"" or data[offset:self.permission_table_entry_size] == b"": return []
            # We have to read the permission table and return it
            permission_table_data = data[offset:self.permission_table_entry_size * entries]
            for entry in range(entries):
    ...
    def write_file(self, file_name: bytes, file_data: bytes, permissions:list=[0x0, 0x1, [None]]) -> int:
        ...
        permission_table = self.permission_table_handler(file_table_offset + entries * self.file_table_entry_size, data, False, entries)
        # Same with the file tree
        file_tree= self.file_tree_handler(file_tree_offset, data, False, file_tree_list)
        # Rebuild both
        if permission_table:
            permission_table_data = struct.pack(PERMISSION_TABLE_ENTRY_FORMAT, *permission_table) + permission_table_data
        if file_tree:
